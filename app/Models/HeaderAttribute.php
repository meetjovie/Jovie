...
class HeaderAttribute extends Model
{
    ...
    public static function updateSortOrder($userId, $newIndex = 0, $oldIndex = 0, $fieldId = null, $listId = null)
    {
        ...
        if (!is_null($fieldId)) {
            if ($newIndex > $oldIndex) {
                ...
                $headerAttribute = HeaderAttribute::where('field_id', $fieldId);
                if (is_null($listId)) {
        ...
        $listOrders = HeaderAttribute::whereIn('field_id', $fieldIds)->orderBy('order');
        if (is_null($listId)) {
    ...
    {
        $headerAttribute = FieldAttribute::where('field_id', $fieldId);
        if (is_null($listId)) {
