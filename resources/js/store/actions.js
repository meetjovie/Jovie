    ...
    return new Promise((resolve, reject) => {
      ContactService.enrichContacts(payload.contact_ids)
        .then((response) => {
          response = response.data;
          if (response.status) {
            context.state.crmRecords
              .filter((record) => response.data.includes(record.id))
              .forEach((record) => {
