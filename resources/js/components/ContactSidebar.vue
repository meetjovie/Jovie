  ...
  <div class="h-screen w-full bg-white dark:bg-jovieDark-900">
    <div
      ...
      <div class="flex-none">
        ...
        <div class="grid grid-cols-3">
          ...
          <div class="col-span-2 mt-6 pl-1 pr-2">
            ...
            <div v-if="contact.category" class="">
              <span
            ...
            <div
              ...
              :class="{
                'line-clamp-5 h-12': expandBio,
                'line-clamp-2 h-8': !expandBio,
              }">
              {{ contact.biography }}
            </div>
          </div>
        ...
        <div class="px-4 py-2">
          <ButtonGroup
            ...
            :text="buttonText"
            :loading="saving"
            :success="contact.saved ?? false"
            @click="saveToCrm()"
            class="w-full rounded-md px-4 py-2 font-bold text-white hover:bg-indigo-600" />
          <div class="flex" v-else>
            <Menu>
              <Float portal :offset="2" placement="bottom-start">
                <MenuButton
                  class="inline-flex items-center rounded border border-slate-300 px-2 py-0.5 text-2xs font-light text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30 dark:border-jovieDark-border dark:border-jovieDark-border dark:text-jovieDark-300 dark:hover:bg-jovieDark-800">
                  <ChatBubbleLeftIcon
                    class="h-3 w-3 text-slate-400 hover:text-slate-500 dark:text-jovieDark-600 dark:hover:text-slate-400"
                    aria-hidden="true" />
                  <span class="line-clamp-1 px-2 text-center">Message</span>
                  <ChevronDownIcon
      ...
      <div class="grow overflow-y-scroll">
        <div class="mb-2 mt-4 flex w-full items-center justify-between px-2">
        ...
        <div v-else class="relative h-full">
          <div
            ...
            :class="{
              ' blur-sm saturate-150 ': !contact.id,
            }">
                ...
                <div
                  ...
                  <template v-else>
                    <template v-if="element.name == 'Address'">
                      <template
                        v-for="addressKey in Object.keys(
                          contact[element.model]
                        )">
                        <DataInputGroup
                          @copy="
                            copyToClipboard(contact[element.model][addressKey])
                          "
      ...
      <div
        ...
        <div class="flex h-8 justify-between">
          <span
            ...
            >Last update by: {{ contact.description_updated_by }} {{ contact.description_updated_at }}</span
          >
          <span
        ...
        <div
          ...
          <div class="mx-auto w-full max-w-sm lg:w-96">
            <div>
              ...
              <h2
                class="mt-6 text-3xl font-extrabold text-slate-100 text-slate-900">
                Sign in
              </h2>
...
export default {
  ...
  data() {
    return {
      ...
      closeSidebar: '',
      imageLoaded: true,
      //add hiddenFields, list an 3 fields, each with a name and icon property. phone, email, and website
      hiddenFields: [
        {
          name: 'Phone',
          icon: 'PhoneIcon',
  ...
  methods: {
    ...
    getFields() {
      FieldService.getFields(this.contact.id)
        .then((response) => {
    ...
    saveToCrm() {
      this.saving = true;
      UserService.saveToCrm(this.contact)
        .then((response) => {
          response = response.data;
          if (response.status) {
