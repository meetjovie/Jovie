<template>
    <div>
        ...
        <div class="mt-1 relative  only:rounded-md  first:rounded-l-md last:rounded-r-md shadow-sm">
            ...
            <input :type="type" :name="name" :id="name" :disabled="disabled" :value="modelValue ?? value" @input="$emit('update:modelValue', $event.target.value)"
                   class="focus-visible:ring-indigo-500 focus-visible:border-indigo-500 block w-full text-neutral-700 py-2 text-sm border-gray-300  only:rounded-md  first:rounded-l-md last:rounded-r-md"
                   :class="[icon ? 'pl-10' : '']"
